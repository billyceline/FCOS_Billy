...
if(FLAGS.use_PANET == False):
    ...
    boxes =  tf.compat.v1.placeholder(tf.float32,[None,3200,4],'gt_boxes')
    classes = tf.compat.v1.placeholder(tf.float32,[None,3200,FLAGS.num_class],'gt_classes')
    centerness = tf.compat.v1.placeholder(tf.float32,[None,3200,1],'gt_centerness')
    state = tf.compat.v1.placeholder(tf.float32,[None,3200],'gt_state') 
   
if(FLAGS.use_PANET == True):
    feature_size = [(50,64)]
    feature_layer_list = ['P']
    stride=[16]
else:
