...
def encode_boxes_PANET(annotation_batch,cls_batch,feature_size,stride,concatenate=True):
    ymin_true = np.expand_dims(annotation_batch[...,0],axis=2)#(b,N,1)
    ymax_true = np.expand_dims(annotation_batch[...,2],axis=2)
    xmin_true = np.expand_dims(annotation_batch[...,1],axis=2)
    xmax_true = np.expand_dims(annotation_batch[...,3],axis=2)
    ...
    y_center_mapping = np.array([(i*stride[0]+offset) for i in range(feature_size[0][0])])/FLAGS.image_height #[w]
    x_center_mapping = np.array([(i*stride[0]+offset) for i in range(feature_size[0][1])])/FLAGS.image_width#[H]
...
#calculate top,bottom,left,right and concate together  
    ...
    true_classes = np.expand_dims(np.expand_dims(cls_batch,axis=2),axis=3)
    #encode true boxes to feature map
    true_boxes = true_boxes * xy_mask#(?,?,100,128,4)
    true_classes = true_classes * xy_mask
    tblr = tblr *xy_mask
    ...
    for b in range(FLAGS.batch_size):
        w_boxes=[]
        w_classes=[]
        for w in range(feature_size[0][0]):
            h_boxes = []
            h_classes=[]
    ...
    lr_max = np.max(pos_true_boxes[...,2:],axis=-1)
    lr_min = np.min(pos_true_boxes[...,2:],axis=-1)
    centerness = np.sqrt((lr_min/(lr_max+1e-8)) * (tb_min/(tb_max+1e-8)))
    matched_true_boxes=np.reshape(pos_true_boxes,(-1,feature_size[0][0]*feature_size[0][1],4))
    matched_true_classes=np.reshape(pos_true_classes,(-1,feature_size[0][0]*feature_size[0][1],FLAGS.num_class))
    matched_true_centerness=np.reshape(centerness,(-1,feature_size[0][0]*feature_size[0][1],1))
...
# stride=[8,16,32,64,128]
    ...
    localization_pred = localization_pred*np.array([FLAGS.image_height,FLAGS.image_height,FLAGS.image_width,FLAGS.image_width])
    localization_mask = tf.cast(tf.expand_dims((tf.reduce_min(localization_pred,axis=-1)>1),axis=-1),tf.float32)
    localization_pred = localization_pred * localization_mask
    centerness_pred  = centerness_pred *localization_mask
    classes_pred = classes_pred * localization_mask
